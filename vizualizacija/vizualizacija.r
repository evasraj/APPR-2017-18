...
#     g <- g + geom_point(data = data.frame(x=0, y=0), aes(x=x, y=y), color = "gray", shape = 1, size = 5)
#     g <- g + geom_point(data = data.frame(x=c(-3 * pi / 2, pi / 2), y=c(1, 1)), aes(x=x, y=y), color = "red", shape = 2, size = 4)
#     g <- g + geom_point(data = data.frame(x=c(-pi / 2, 3 * pi / 2), y=c(-1, -1)), aes(x=x, y=y), color = "green", shape = 6, size = 4)
# 
